...
public class ThreadUtils {
    ...
    public static boolean awaitUninterruptibly(CountDownLatch countDownLatch, long j5) {
        ...
        handler.post(new Runnable() { // from class: org.webrtc.ThreadUtils.3
            ...
            public void run() {
                try {
                    ...
                    c1CaughtException.f8028e = e10;
                }
                countDownLatch.countDown();
            }
        });
        awaitUninterruptibly(countDownLatch);
        Exception exc = c1CaughtException.f8028e;
        if (exc == null) {
            return c1Result.value;
        }
        RuntimeException runtimeException = new RuntimeException(exc);
        runtimeException.setStackTrace(concatStackTraces(c1CaughtException.f8028e.getStackTrace(), runtimeException.getStackTrace()));
        throw runtimeException;
    }

    public static boolean joinUninterruptibly(Thread thread, long j5) {
        long elapsedRealtime = SystemClock.elapsedRealtime();
        boolean z8 = false;
        long j9 = j5;
        while (j9 > 0) {
