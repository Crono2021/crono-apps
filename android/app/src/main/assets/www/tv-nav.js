...
(function () {
  ...
  class CineflixTVNav {
    ...
    getZones() {
      ...
      // Zone 2 — Main nav (Series / Películas / Plataformas tabs + Refresh)
      // Exclude platform-subnav buttons — they get their own zone below
      const navBtns = [...view.querySelectorAll('.main-nav-btn, .btn-refresh-nav')]
        .filter(el => this.visible(el) && !el.closest('#platform-subnav'));
      if (navBtns.length) zones.push({ key: 'main-nav', items: navBtns });

      // Zone 3 — Platform cards (must come before genre tabs / hero so D-pad reaches them)
      const platformCards = [...view.querySelectorAll('.platform-card')].filter(el => this.visible(el));
      if (platformCards.length) {
          const itemsPerRow = 4;
          for (let i = 0; i < platformCards.length; i += itemsPerRow) {
              const chunk = platformCards.slice(i, i + itemsPerRow);
              zones.push({ key: `platforms-row-${i}`, items: chunk });
          }
      }

      // Zone — Back button (must come before genre tabs so it's reachable from platform content)
      const backBtn = view.querySelector('.btn-back');
      if (backBtn && this.visible(backBtn)) {
        zones.push({ key: 'back', items: [backBtn] });
      }

      // Zone — Platform sub-nav (📺 Series | 🎬 Películas inside platform content)
      const platformSubNav = view.querySelector('#platform-subnav');
      if (platformSubNav) {
        const subNavBtns = [...platformSubNav.querySelectorAll('.main-nav-btn')].filter(el => this.visible(el));
        if (subNavBtns.length) zones.push({ key: 'platform-subnav', items: subNavBtns });
      }
      ...
      [...view.querySelectorAll('.content-row')].forEach((row, i) => {
        const viewAllBtn = row.querySelector('.btn-view-all');
        if (viewAllBtn && this.visible(viewAllBtn)) {
            zones.push({ key: `row-header-${i}`, items: [viewAllBtn] });
        }
      ...
      const gridCards = [...view.querySelectorAll(
        '#catalog-grid .series-card, #movies-grid .series-card, .catalog-grid .series-card'
      )].filter(el => this.visible(el));
      if (gridCards.length) {
          // Chunk into rows (assume roughly 4-6 cards per row based on CSS grid, let's use 5 as a safe average, or just calculate dynamically? Static chunk is safer for TV nav)
          // The CSS grid is usually auto-fill or 4 columns. Let's use 4.
          const itemsPerRow = 4;
          for (let i = 0; i < gridCards.length; i += itemsPerRow) {
              const chunk = gridCards.slice(i, i + itemsPerRow);
              zones.push({ key: `grid-row-${i}`, items: chunk });
          }
      }

      // (Platform cards zone is already handled above, near the top)
    ...
    move(direction) {
      ...
      if (direction === 'left') {
        ...
        // Transfer horizontal position for true spatial navigation
        const targetZone = zones[this.zoneIndex];
        this.lastRowIndex[targetZone.key] = Math.min(prevIdx, targetZone.items.length - 1);
    ...
    confirm() {
      ...
      if (this.focused.tagName === 'INPUT' || this.focused.tagName === 'TEXTAREA') {
        ...
        this.focused.focus();
        this.focused.click(); // Forzar evento de click para Fire OS / WebViews antiguos
    ...
    visible(el) {
      ...
      // Si el elemento está explícitamente oculto por CSS, no es visible
      if (cs.display === 'none' || cs.visibility === 'hidden' || cs.opacity === '0') {
          return false;
      }
      
      // En WebViews de TV (Fire Stick), los elementos muy abajo en scroll infinito 
      // pueden reportar dimensiones 0x0. Si tienen offsetParent, están en el DOM activo.
